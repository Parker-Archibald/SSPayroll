    ...
    <Popover className="relative">
      ...
      >
        <div className="w-full shrink rounded-xl bg-white p-4 text-sm/6 font-semibold text-gray-900 shadow-lg ring-1 ring-gray-900/5">
          {solutions.map((item) => (
            <CloseButton key={item.str} className="block hover:text-blue-600 w-full" onClick={() => {callBack(item.name); setOption(item.str)}}>
              {item.str === option ? <p className='bg-gray-100 py-2 px-2 rounded-md text-left'>{item.str}</p> : <p className='py-2 px-2 text-left'>{item.str}</p>}
            </CloseButton>
          ))}
        </div>
      </PopoverPanel>
